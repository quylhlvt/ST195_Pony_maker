...
class AddCharacterViewModel @Inject constructor(
     ...
     */
    suspend fun loadDataFromMainViewModel(
        ...
        withContext(Dispatchers.Default) {
            ...
            try {
                // Convert backgrounds từ ViewModelActivity
                backgroundImageList.clear()
                backgroundImageList.addAll(
                    backgrounds.map { SelectedAddModel(path = it) }
                )

                // Background Colors từ DataLocal
                backgroundColorList.clear()
                backgroundColorList.addAll(DataLocal.getBackgroundColorDefault(context))

                // Convert stickers từ ViewModelActivity
                stickerList.clear()
                stickerList.addAll(
                    stickers.map { SelectedAddModel(path = it) }
                )

                // Convert speeches từ ViewModelActivity
                speechList.clear()
                speechList.addAll(
                    speeches.map { SelectedAddModel(path = it) }
                )

                // Text Fonts từ DataLocal
                textFontList.clear()
                textFontList.addAll(DataLocal.getTextFontDefault())
                textFontList.firstOrNull()?.isSelected = true

                // Text Colors từ DataLocal
                textColorList.clear()
