...
class BackGroundViewModel : ViewModel() {
    ...
    suspend fun loadDataDefault(context: Context) {
        ...
        backgroundImageList.addAll(
            DataHelper.arrBg.map { SelectedModel(path = it) }
           )

        backgroundColorList.clear()
        backgroundColorList.addAll(DataHelper.getBackgroundColorDefault(context))


        stickerList.clear()
        stickerList.addAll(
            DataHelper.arrStiker.map { SelectedModel(path = it) })


        speechList.clear()
        speechList.addAll(
            DataHelper.arrBgText.map { SelectedModel(path = it) })
        ...
        textColorList.clear()
        textColorList.addAll(DataHelper.getTextColorDefault(context))
